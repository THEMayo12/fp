...
def residual(pars, x, data=None):
    ...
    model = X_sig_fit_squared((np.deg2rad(x), 0), z2, n2, n3, sig1, sig2)
    if data is None:
        return model
    return (model - data)
...
theta_test = np.linspace(0.25, 2.5, 100)
...
p_fit = Parameters()
p_fit.add('z2', value=z2)
p_fit.add('n2', value=n2, max=1.0)
p_fit.add('n3', value=n3)
p_fit.add('sig1', value=sig1)
p_fit.add('sig2', value=sig2)
...
fit = residual(p_fit, theta_fit)
print fit_report(p_fit)

s_z2.set_val(-p_fit["z2"].value * 1e10)
s_n2.set_val(1.0 - p_fit["n2"])
s_n3.set_val(1.0 - p_fit["n3"])
s_sig1.set_val(p_fit["sig1"])
s_sig2.set_val(p_fit["sig2"])
